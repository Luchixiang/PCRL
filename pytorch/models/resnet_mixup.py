...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, low_dim=128, in_channel=3, width=1):
        ...
        self.fc1 = nn.Linear(self.base * 8 * block.expansion, self.base * 8 * block.expansion)
        self.fc2 = nn.Linear(self.base * 8 * block.expansion, low_dim)
...
def resnet18(pretrained=False, **kwargs):
    ...
    if pretrained:
        ...
        unParalled_state_dict = {k: v for k, v in state_dict.items() if k in encoder_dict and 'fc' not in k}
        encoder_dict.update(unParalled_state_dict)
...
class InsResNet101(nn.Module):
    ...
    def __init__(self, width=1):
        ...
        self.encoder = resnet101(width=width)
        # self.encoder = nn.DataParallel(self.encoder)
